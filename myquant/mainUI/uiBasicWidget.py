...
from ..utils import *
from .uiQt import BASIC_FONT
from collections import OrderedDict
from ..utils import jsonPathDict
from qtpy import QtWidgets, QtGui, QtCore
from myquant.dataStruct import VtCancelOrderReq, VtSubscribeReq, VtOrderReq
...
class BasicMonitor(QtWidgets.QTableWidget):
    ...
    def saveToCsv(self):
        ...
        # 获取想要保存的文件名
        path = QtWidgets.QFileDialog.getSaveFileName(self, SAVE_DATA, '', 'CSV(*.csv)')
    ...
    def initMenu(self):
        ...
        resizeAction = QtWidgets.QAction(RESIZE_COLUMNS, self)
        resizeAction.triggered.connect(self.resizeColumns)

        saveAction = QtWidgets.QAction(SAVE_DATA, self)
...
class MarketMonitor(BasicMonitor):
    ...
    def __init__(self, mainEngine, eventEngine, parent=None):
        ...
        d['lastPrice'] = {'chinese': LAST_PRICE, 'cellType': BasicCell}
        d['preClosePrice'] = {'chinese': PRE_CLOSE_PRICE, 'cellType': BasicCell}
        d['volume'] = {'chinese': VOLUME, 'cellType': BasicCell}
        d['openInterest'] = {'chinese': OPEN_INTEREST, 'cellType': BasicCell}
        d['openPrice'] = {'chinese': OPEN_PRICE, 'cellType': BasicCell}
        d['highPrice'] = {'chinese': HIGH_PRICE, 'cellType': BasicCell}
        d['lowPrice'] = {'chinese': LOW_PRICE, 'cellType': BasicCell}
        d['bidPrice1'] = {'chinese': BID_PRICE_1, 'cellType': BidCell}
        d['bidVolume1'] = {'chinese': BID_VOLUME_1, 'cellType': BidCell}
        d['askPrice1'] = {'chinese': ASK_PRICE_1, 'cellType': AskCell}
        d['askVolume1'] = {'chinese': ASK_VOLUME_1, 'cellType': AskCell}
        d['time'] = {'chinese': TIME, 'cellType': BasicCell}
...
class PositionMonitor(BasicMonitor):
    ...
    def __init__(self, mainEngine, eventEngine, parent=None):
        ...
        d['price'] = {'chinese': PRICE, 'cellType': BasicCell}
        d['positionProfit'] = {'chinese': POSITION_PROFIT, 'cellType': PnlCell}
        d['gatewayName'] = {'chinese': GATEWAY, 'cellType': BasicCell}
        self.setHeaderDict(d)

        self.setDataKey('vtPositionName')
        self.setEventType(Event.POSITION)
        self.setFont(BASIC_FONT)
        self.setSaveData(True)
...
class AccountMonitor(BasicMonitor):
    ...
    def __init__(self, mainEngine, eventEngine, parent=None):
        ...
        self.setEventType(Event.ACCOUNT)
        self.setFont(BASIC_FONT)
        self.initTable()
        self.registerEvent()
...
class TradingWidget(QtWidgets.QFrame):
    ...
    def initUi(self):
        ...
        # 左边部分
        labelSymbol = QtWidgets.QLabel(CONTRACT_SYMBOL)
        labelName = QtWidgets.QLabel(CONTRACT_NAME)
        labelDirection = QtWidgets.QLabel(DIRECTION)
        labelOffset = QtWidgets.QLabel(OFFSET)
        labelPrice = QtWidgets.QLabel(PRICE)
        self.checkFixed = QtWidgets.QCheckBox(u'')  # 价格固定选择框
        labelVolume = QtWidgets.QLabel(VOLUME)
        labelPriceType = QtWidgets.QLabel(PRICE_TYPE)
        labelExchange = QtWidgets.QLabel(EXCHANGE)
        labelCurrency = QtWidgets.QLabel(CURRENCY)
        labelProductClass = QtWidgets.QLabel(PRODUCT_CLASS)
        labelGateway = QtWidgets.QLabel(GATEWAY)
        ...
        # 右边部分
        labelBid1 = QtWidgets.QLabel(BID_1)
        labelBid2 = QtWidgets.QLabel(BID_2)
        labelBid3 = QtWidgets.QLabel(BID_3)
        labelBid4 = QtWidgets.QLabel(BID_4)
        labelBid5 = QtWidgets.QLabel(BID_5)

        labelAsk1 = QtWidgets.QLabel(ASK_1)
        labelAsk2 = QtWidgets.QLabel(ASK_2)
        labelAsk3 = QtWidgets.QLabel(ASK_3)
        labelAsk4 = QtWidgets.QLabel(ASK_4)
        labelAsk5 = QtWidgets.QLabel(ASK_5)
        ...
        labelLast = QtWidgets.QLabel(LAST)
        ...
        # 发单按钮
        buttonSendOrder = QtWidgets.QPushButton(SEND_ORDER)
        buttonCancelAll = QtWidgets.QPushButton(CANCEL_ALL)
    ...
    def updateSymbol(self):
        ...
        # 重新注册事件监听
        self.eventEngine.unregister(Event.TICK + self.symbol, self.signal.emit)
        self.eventEngine.register(Event.TICK + vtSymbol, self.signal.emit)
...
class ContractMonitor(BasicMonitor):
    ...
    def __init__(self, mainEngine, parent=None):
        ...
        d = OrderedDict()
        d['symbol'] = {'chinese': CONTRACT_SYMBOL, 'cellType': BasicCell}
        d['exchange'] = {'chinese': EXCHANGE, 'cellType': BasicCell}
        d['vtSymbol'] = {'chinese': VT_SYMBOL, 'cellType': BasicCell}
        d['name'] = {'chinese': CONTRACT_NAME, 'cellType': BasicCell}
        d['productClass'] = {'chinese': PRODUCT_CLASS, 'cellType': BasicCell}
        d['size'] = {'chinese': CONTRACT_SIZE, 'cellType': BasicCell}
        d['priceTick'] = {'chinese': PRICE_TICK, 'cellType': BasicCell}

        d['underlyingSymbol'] = {'chinese': UNDERLYING_SYMBOL, 'cellType': BasicCell}
        d['optionType'] = {'chinese': OPTION_TYPE, 'cellType': BasicCell}
        d['expiryDate'] = {'chinese': EXPIRY_DATE, 'cellType': BasicCell}
        d['strikePrice'] = {'chinese': STRIKE_PRICE, 'cellType': BasicCell}
        self.setHeaderDict(d)
...
class ContractManager(QtWidgets.QWidget):
    ...
    def initUi(self):
        """初始化界面"""
        self.setWindowTitle(CONTRACT_SEARCH)

        self.lineFilter = QtWidgets.QLineEdit()
        self.buttonFilter = QtWidgets.QPushButton(SEARCH)
...
class SettingEditor(QtWidgets.QWidget):
    ...
    def initUi(self):
        """初始化界面"""
        self.setWindowTitle(EDIT_SETTING)
