...
from ...utils import getJsonPath, getTempPath
from myquant.dataStruct import VtSubscribeReq, VtOrderReq, VtCancelOrderReq, VtLogData
from ...constant import DIRECTION_LONG, DIRECTION_SHORT, OFFSET_OPEN, OFFSET_CLOSE, PRICETYPE_LIMITPRICE

from .stBase import StLeg, StSpread
...
class StDataEngine(object):
    ...
    def putSpreadTickEvent(self, spread):
        """发出价差行情更新事件"""
        event1 = Event(Event.SPREADTRADING_TICK + spread.name)
        event1.dict_['data'] = spread
        self.eventEngine.put(event1)

        event2 = Event(Event.SPREADTRADING_TICK)
    ...
    def processTradeEvent(self, event):
        ...
        # 推送价差持仓更新
        event1 = Event(Event.SPREADTRADING_POS + spread.name)
        event1.dict_['data'] = spread
        self.eventEngine.put(event1)

        event2 = Event(Event.SPREADTRADING_POS)
        event2.dict_['data'] = spread
        self.eventEngine.put(event2)
    ...
    def registerEvent(self):
        """"""
        self.eventEngine.register(Event.TICK, self.processTickEvent)
        self.eventEngine.register(Event.TRADE, self.processTradeEvent)
        self.eventEngine.register(Event.POSITION, self.processPosEvent)
    ...
    def writeLog(self, content):
        ...
        event = Event(Event.SPREADTRADING_LOG)
...
class StAlgoEngine(object):
    ...
    def registerEvent(self):
        """注册事件监听"""
        self.eventEngine.register(Event.SPREADTRADING_TICK, self.processSpreadTickEvent)
        self.eventEngine.register(Event.SPREADTRADING_POS, self.processSpreadPosEvent)
        self.eventEngine.register(Event.TRADE, self.processTradeEvent)
        self.eventEngine.register(Event.ORDER, self.processOrderEvent)
        self.eventEngine.register(Event.TIMER, self.processTimerEvent)
    ...
    def putAlgoEvent(self, algo):
        """发出算法状态更新事件"""
        event = Event(Event.SPREADTRADING_ALGO + algo.name)
        self.eventEngine.put(event)
    ...
    def writeLog(self, content):
        ...
        event = Event(Event.SPREADTRADING_ALGOLOG)
        event.dict_['data'] = log
