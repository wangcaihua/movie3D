...
from .text import *
from myquant.mainUI.uiBasicWidget import QtWidgets
...
class RmEngineManager(QtWidgets.QWidget):
    ...
    def initUi(self):
        """初始化界面"""
        self.setWindowTitle(RISK_MANAGER)

        # 设置界面
        self.buttonSwitchEngineStatus = QtWidgets.QPushButton(RISK_MANAGER_STOP)
        ...
        buttonClearOrderFlowCount = QtWidgets.QPushButton(CLEAR_ORDER_FLOW_COUNT)
        buttonClearTradeCount = QtWidgets.QPushButton(CLEAR_TOTAL_FILL_COUNT)
        buttonSaveSetting = QtWidgets.QPushButton(SAVE_SETTING)

        Label = QtWidgets.QLabel
        grid = QtWidgets.QGridLayout()
        grid.addWidget(Label(WORKING_STATUS), 0, 0)
        grid.addWidget(self.buttonSwitchEngineStatus, 0, 1)
        grid.addWidget(RmLine(), 1, 0, 1, 2)
        grid.addWidget(Label(ORDER_FLOW_LIMIT), 2, 0)
        grid.addWidget(self.spinOrderFlowLimit, 2, 1)
        grid.addWidget(Label(ORDER_FLOW_CLEAR), 3, 0)
        grid.addWidget(self.spinOrderFlowClear, 3, 1)
        grid.addWidget(RmLine(), 4, 0, 1, 2)
        grid.addWidget(Label(ORDER_SIZE_LIMIT), 5, 0)
        grid.addWidget(self.spinOrderSizeLimit, 5, 1)
        grid.addWidget(RmLine(), 6, 0, 1, 2)
        grid.addWidget(Label(TOTAL_TRADE_LIMIT), 7, 0)
        grid.addWidget(self.spinTradeLimit, 7, 1)
        grid.addWidget(RmLine(), 8, 0, 1, 2)
        grid.addWidget(Label(WORKING_ORDER_LIMIT), 9, 0)
        grid.addWidget(self.spinWorkingOrderLimit, 9, 1)
        grid.addWidget(RmLine(), 10, 0, 1, 2)
        grid.addWidget(Label(CONTRACT_CANCEL_LIMIT), 11, 0)
        grid.addWidget(self.spinOrderCancelLimit, 11, 1)
        grid.addWidget(RmLine(), 12, 0, 1, 2)
        grid.addWidget(Label(MARGIN_RATIO_LIMIT), 13, 0)
    ...
    def updateEngineStatus(self):
        ...
        if self.rmEngine.active:
            self.buttonSwitchEngineStatus.setText(RISK_MANAGER_RUNNING)
        else:
            self.buttonSwitchEngineStatus.setText(RISK_MANAGER_STOP)
